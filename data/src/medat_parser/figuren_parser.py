...
Output per question (flat directory):
  NNN_question.png  — cropped figure image
  NNN_question.txt  — minimal description
  NNN_solution.txt  — answer letter
  solutions/        — full-page solution renders
"""
...
def parse_figuren(pdf_path: Path, output_dir: Path) -> None:
    ...
    for page_idx in question_pages:
        ...
        for i, (qnum, qtop, _qbottom) in enumerate(q_items):
            if i < len(q_items) - 1:
                ...
                question_bottom = next_top - 5
            else:
                question_bottom = page.height - 10

            questions_in_page.append(
                {"qnum": qnum, "top": qtop, "bottom": question_bottom}
            )
    ...
    # Render pages and split using PyMuPDF
    ...
    for page_idx, q_items in all_questions:
        ...
        for q in q_items:
            ...
            y0 = max(0, int(q["top"] * scale))
            y1 = min(pix.height, int(q["bottom"] * scale))

            # Crop and save question image
            full_img = Image.open(str(page_png_path))
            cropped = full_img.crop((0, y0, full_img.width, y1))
            cropped.save(str(output_dir / f"{global_id:03d}_question.png"))
            full_img.close()

            answer = answers.get(global_id, "")

            # Write question text
            (output_dir / f"{global_id:03d}_question.txt").write_text(
                f"Figuren zusammensetzen\nFrage {global_id}\n", encoding="utf-8"
            )

            # Write solution
            (output_dir / f"{global_id:03d}_solution.txt").write_text(
                f"{answer}\n", encoding="utf-8"
            )
    ...
    total = len(list(output_dir.glob("*_question.png")))
    print(f"Figuren: {total} questions")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Parse Figuren zusammensetzen PDF")
